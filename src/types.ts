...
export function createEventEmitter<T>(): EventEmitter<T> {
  ...
  const subscribe = (handler: EventHandler<T>): Unsubscribe => {
 ...
 * ```
 */
export function withMetadata<T extends StructuredTool>(
  tool: T,
  metadata: ToolEnhancement
): T & EnhancedTool {
  const enhanced = tool as T & EnhancedTool;
