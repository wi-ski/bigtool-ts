...
/**
 * JSON Schema type definitions (subset for tool parameters).
 */
 ...
 */
function convertSchema(schema: JSONSchemaObject): z.ZodTypeAny {
  ...
  if (schema.enum && Array.isArray(schema.enum) && schema.enum.length > 0) {
    ...
    if (enumValues.every((v) => typeof v === 'string')) {
      ...
      if (stringValues.length >= 1) {
        const result = z.enum([stringValues[0], ...stringValues.slice(1)] as [string, ...string[]]);
    ...
    if (enumValues.length >= 2) {
      ...
      const result = z.union([literals[0], literals[1], ...literals.slice(2)] as [z.ZodTypeAny, z.ZodTypeAny, ...z.ZodTypeAny[]]);
      return schema.description ? result.describe(schema.description) : result;
    } else if (enumValues.length === 1) {
      // Single value enum: just use literal
      const result = z.literal(enumValues[0] as z.Primitive);
      return schema.description ? result.describe(schema.description) : result;
    }
  }
 ...
 */
function convertString(schema: JSONSchemaObject): z.ZodTypeAny {
 ...
 */
function convertNumber(schema: JSONSchemaObject): z.ZodTypeAny {
 ...
 */
function convertArray(schema: JSONSchemaObject): z.ZodTypeAny {
