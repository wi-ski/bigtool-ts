...
import { createEventEmitter, type EventEmitter } from '../types.js';
...
export class DefaultToolCatalog implements ToolCatalog {
  ...
  private metadataBySource: Map<string, ToolMetadata[]> = new Map();

  /** @internal Map of tool ID to metadata for fast lookup */
  private metadataById: Map<string, ToolMetadata> = new Map();
  ...
  async register(source: ToolSource): Promise<void> {
    ...
    if (source.onRefresh) {
      source.onRefresh.on((newMetadata: ToolMetadata[]) => {
   ...
   */
  private handleSourceRefresh(sourceId: string, newMetadata: ToolMetadata[]): void {
